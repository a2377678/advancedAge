...
<body>
  ...
  <div id="main">
    ...
    <div id="content">
      ...
      <!------------------- search ------------------->
      <c:if test="${functionPermission3.indexOf('S01')!=-1 }">
      <form action="s01" method="post" id="searchForm">
	      <div class="case_search-left" id="search_bar">
	          <table class="table_01">
	            ...
	            <tr>
	              ...
	              <th>申請單位名稱：</th>
	              <td>
	              	<input type="text" size="11" id="companyName" name="companyName" value="${base.companyName}">
	              </td>
	            </tr>
	            <tr>
	              <th>縣市別：</th>
		      	...
		      	<c:when test="${not empty baseList}">
			      	<c:forEach items="${baseList}" var="item" varStatus="status">
			      		<c:if test="${item.fileStatus==4 }">
					          ...
					          <c:if test="${item.fileStatus=='4' && item.caseStatus<4}">,'u01_1'</c:if>
					          <c:if test="${item.fileStatus=='4' && (item.caseStatus==4 || item.caseStatus==8)}">,'u02_1'</c:if>
					          <c:if test="${item.fileStatus=='4' && item.caseStatus>4 && item.caseStatus<8}">,'v01_1'</c:if>)">
      ...
      <c:if test="${functionPermission3.indexOf('S01')==-1 }"><p>無此權限</p></c:if>
  </div>
...
<script src="js/s01/s01.js"></script>
