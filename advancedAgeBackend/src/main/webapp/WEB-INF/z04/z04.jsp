...
<body>
  ...
  <div id="main">
    ...
    <div id="content">
      ...
      <!------------------- search ------------------->
      <form action="z04" method="post" id="searchForm">
      <div id="search_bar">
        <table class="table_01">
            ...
            <td width="2%">
            	<select name="unit" id="unit">
		               ...
		               <c:forEach  items="${unitList}"  var="item"  varStatus="userStatus">
			      		<option value="${item.id}" <c:if test="${userLoginRecord.unit==item.id}">selected</c:if> >${item.name}</option>
			      	</c:forEach>
             	</select>
            </td>
            <th width="14%">登入日期：</th>
          ...
          <tr>
            ...
            <th>使用者姓名：</th>
            <td>
            	<input type="text" size="11" id="username" name="username" value="${userLoginRecord.username}"/>
            </td>
            <td>
      ...
      <table class="table_07">
        ...
        <c:choose>
        <c:when test="${not empty userLoginRecordDate}">
...
<script src="js/z04/z04.js"></script>
