...
<script type="text/javascript" src="js/i02/i02.js"></script>
</head>
...
<body>
  ...
  <div id="main">
    ...
    <div id="content">
      ...
      <div id="account">
        ...
        <a href="logout">登出</a>
      	...
      	<c:if test="${functionPermission.indexOf('I02')!=-1 }">
		<!------------------- search ------------------->
		<form action="i02" method="post" id="searchForm">
	      <div id="search_bar">
        ...
        <table class="table_04">
          ...
          <c:choose><c:when test="${not empty caseStatisticsList}">
      		<c:forEach items="${caseStatisticsList}" var="item" varStatus="status">
	          <tr>
	            <td>${item.companyName }</td>
	            <td><c:forEach  items="${industryList}"  var="industryItem"  varStatus="userStatus">
		      			<c:if test="${item.industry==industryItem.code}">${industryItem.name}</c:if>
		      		</c:forEach>
      			</td>
	            <td>${item.nearHighEmploymentNumber}</td>
	            <td>${item.continueEmploymentPercentage}</td>
	            <td></td>
	          </tr>
	          </c:forEach>
	      </c:when></c:choose>
    ...
    <c:if test="${functionPermission.indexOf('I02')==-1 }"><p>無此權限</p></c:if>
    </div>
