...
<script type="text/javascript" src="js/n01/file.js"></script>
</head>
...
<div class="main_box">
    ...
    <div class="file_group">
      ...
      <ul class="file_list-2">
          ...
          <div class="file-preview">
            <c:choose><c:when test="${not empty expenditureAttachment}">
		      	...
		      	<c:forEach items="${expenditureAttachment}" var="item" varStatus="status">
			      	<c:choose>
				      	<c:when test="${item.fileType=='expenditure' && item.fileFrequency == recordStatus.count}">
				      		<c:set var="i" value="${i+1}"/>
				      		<div class="full">
		      			 		<a href="/file${item.filePath}/${item.fileName}" class="visited" target="_blank">
	      			 				<c:if test="${i<10}">0${i}</c:if>
        ...
        <li>
          ...
          <div class="file-check">
            <label for="speakerFeeFileStatus1${recordStatus.count}"><input type="checkbox" name="speakerFeeFileStatus1${recordStatus.count}" id="speakerFeeFileStatus1${recordStatus.count}" value="1" <c:if test="${recordItem.split('、')[4].indexOf('1')!=-1}">checked</c:if>>未附、尚有缺件</label>
            <label for="speakerFeeFileStatus2${recordStatus.count}"><input type="checkbox" name="speakerFeeFileStatus2${recordStatus.count}" id="speakerFeeFileStatus2${recordStatus.count}" value="2" <c:if test="${recordItem.split('、')[4].indexOf('2')!=-1}">checked</c:if>>不清晰、無法瀏覽</label>
            <label for="speakerFeeFileStatus3${recordStatus.count}"><input type="checkbox" name="speakerFeeFileStatus3${recordStatus.count}" id="speakerFeeFileStatus3${recordStatus.count}" value="3" <c:if test="${recordItem.split('、')[4].indexOf('3')!=-1}">checked</c:if>>格式不符、內容有誤</label>
            <label for="speakerFeeFileStatus4${recordStatus.count}"><input type="checkbox" name="speakerFeeFileStatus4${recordStatus.count}" id="speakerFeeFileStatus4${recordStatus.count}" value="4" <c:if test="${recordItem.split('、')[4].indexOf('4')!=-1}">checked</c:if>>符合</label>
          </div>
        </li>
        ...
        <li>
          ...
          <div class="file-check">
            <label for="necessaryFileStatus1${recordStatus.count}"><input type="checkbox" name="necessaryFileStatus1${recordStatus.count}" id="necessaryFileStatus1${recordStatus.count}" value="1" <c:if test="${recordItem.split('、')[5].indexOf('1')!=-1}">checked</c:if>>未附、尚有缺件</label>
            <label for="necessaryFileStatus2${recordStatus.count}"><input type="checkbox" name="necessaryFileStatus2${recordStatus.count}" id="necessaryFileStatus2${recordStatus.count}" value="2" <c:if test="${recordItem.split('、')[5].indexOf('2')!=-1}">checked</c:if>>不清晰、無法瀏覽</label>
            <label for="necessaryFileStatus3${recordStatus.count}"><input type="checkbox" name="necessaryFileStatus3${recordStatus.count}" id="necessaryFileStatus3${recordStatus.count}" value="3" <c:if test="${recordItem.split('、')[5].indexOf('3')!=-1}">checked</c:if>>格式不符、內容有誤</label>
            <label for="necessaryFileStatus4${recordStatus.count}"><input type="checkbox" name="necessaryFileStatus4${recordStatus.count}" id="necessaryFileStatus4${recordStatus.count}" value="4" <c:if test="${recordItem.split('、')[5].indexOf('4')!=-1}">checked</c:if>>符合</label>
          </div>
        </li>
      </ul>
      ...
      <div class="upload-date">
        <span>上傳日期：
          ...
          <span class="time">${recordItem.split('、')[6].substring(recordItem.split('、')[6].indexOf(" ")+1,recordItem.split('、')[6].length())}</span>
          <span class="date">${recordItem.split('、')[6].substring(0,4)-1911}${recordItem.split('、')[6].substring(4,recordItem.split('、')[6].indexOf(" "))}</span>
		  <input type="text" id="uploadTime${recordStatus.count}" name="uploadTime${recordStatus.count}" value="${recordItem.split('、')[6]}" style="display:none">
        </span>
      </div>
  ...
  <div class="mail_box">
    <div class="mail_text" id="mailHeader">
      臺端申請本部${apply.applyYear}年度僱用退休中高齡者及高齡者傳承專業技術及經驗補助計畫請領作業，經查核附件尚有缺件或不符規定。需補附件項目及原因如下：
    </div>
    ...
    <div class="mail_text" id="mailFooter">
        補件網址：<a href="${mailDomain}/advancedAgeFrontend/teach_payment">${mailDomain}/advancedAgeFrontend/teach_payment</a><br>
