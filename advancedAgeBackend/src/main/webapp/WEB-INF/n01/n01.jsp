...
<script type="text/javascript" src="js/n01/n01.js"></script>
...
<body>
  ...
  <div id="main">
    ...
    <div id="content">
	  ...
	  <c:if test="${functionPermission2.indexOf('N01')!=-1 }">
      <!------------------- search ------------------->
      <form action="n01" method="post" id="searchForm">
      <div id="search_bar">
      ...
      <ol>
        <c:choose><c:when test="${not empty baseList}">
	      	<c:forEach items="${baseList}" var="item" varStatus="status">
		          ...
		          <span class="update_type"><c:choose><c:when test="${item.caseStatus=='5'}">待核銷</c:when><c:when test="${item.caseStatus=='6'}">核銷不合格</c:when><c:when test="${item.caseStatus=='7'}">已核銷</c:when></c:choose></span>
		          <span class="district">
      ...
      <form action="n01_1" method="post" id="dataForm">
      ...
      <c:if test="${functionPermission2.indexOf('N01')==-1 }"><p>無此權限</p></c:if>
