$(function(){
	...
	$('button.btn_a1s').click(function(){
		...
		$('div.file_group').each(function(){
			...
			//講師鐘點費印領清冊
			$(this).find('input[name^="speakerFeeFileStatus"]:checked').each(function(){
				if(speakerFee==''){
					speakerFee=$(this).val();
				}else{
					speakerFee=speakerFee+$(this).val();
				}
			})
			if(speakerFee == ''){
				final=final+"、4";
			}else{
				final=final+"、"+speakerFee;
			}
		...
		msg='';
		$(this).parent().parent().parent().parent().find('input[name^="speakerFeeFileStatus"]:checked').each(function(){
			if(msg==''){
				if($(this).val()!=4){
					msg=$(this).parent().text();
				}
