...
<!-- 	<div id="countDown" class="countdown shadow-sm">計時提醒 30:00</div> -->
    <!---無障礙/麵包屑--->
    ...
    <div class="form">
      ...
      <input type="text" pattern="\d{8}" minlength="8" maxlength="8" id="seq" name="seq" <c:if test="${empty companyInfoData.get('seq')}">value="${apply.seq}"</c:if><c:if test="${not empty companyInfoData.get('seq')}">value="${companyInfoData.get('seq')}"</c:if> readonly>
      </div>
        
      <div>
      <label for="select">行業別</label>
      ...
      <div>
        ...
        <select name="companyAttributes" id="companyAttributes" required>
          <option value>請選擇</option>
          <option value="1" <c:if test="${apply.companyAttributes==1}">selected</c:if>>民營事業單位</option>
          <option value="2" <c:if test="${apply.companyAttributes==2}">selected</c:if>>民間團體</option>
          <option value="3" <c:if test="${apply.companyAttributes==3}">selected</c:if>>私立學校</option>
        </select>
      </div>
      ...
      <div class="affidavit">
        ...
        <ol>
          <li>
          <label class="title" for="q01">本統一編號/非營利扣繳編號是否有其他子公司或母公司？(有母子公司人數請合併計算)</label>
          ...
          <li>
          <label class="title" for="q04"><strong id="docs-internal-guid-b2fca299-7fff-67e7-359d-8c98ca11bc5f">本單位申請本計畫時曾領取政府機關與本計畫屬性相同之津貼</strong>？</label>
...
<script src="js/career_02/career_02.js"></script>
