...
<div class=""> 
  ...
  <div class="main clearfix" id="content"> 
    ...
    <div class="apply_main">
      ...
      <h2>申請流程 Step7. 核對資料</h2>
      
      <!---申辦前準備--->
      <h3>請核對您填寫的資料，確認並「送出案件」<br>
        送出案件後不能修改，可在案件查詢再次下載及列印</h3>
      ...
      <div class="form">
        ...
        <div><span class="title-c">單位屬性</span>${companyAttributes}</div>
        <div><span class="title-c">勞保投保證號</span>${apply.guaranteeNumber.replace(";","、")}</div>
        <div class="full">
        	<span class="title-c">
        	聯絡地址</span><c:forEach items="${cityList}" var="item"><c:if test="${item.code==apply.contactCity}">${item.name}</c:if></c:forEach><c:forEach items="${areaList}" var="item"><c:if test="${item.code==apply.contactArea}">${item.name}</c:if></c:forEach>${apply.contactAddress}
      ...
      <div class="form">
        ...
        <div class="ss"><span class="title-c">員工總人數</span>${plan.employeeNumber}</div>
        <div class="ss"><span class="title-c">計畫受益人數</span>${plan.beneficiaryNumber}</div>
        <div class="ss"><span class="title-c">申請金額</span>${plan.amounts}</div>
        <div class="full"><span class="title-c">申請補助類型</span>${subsidyType}</div>
        <div class="full"><span class="title-c">經費結構 (1)</span>同一計畫向二個以上政府機關提出申請補助之項目與金額－(${multipleApplications}) ${plan.multipleApplicationsDescription}</div>
        <div class="full"><span class="title-c">經費結構 (2)</span>同年度申請中高齡者退休後再就業準備訓練補助實施計畫－(${applySameYear}) ${plan.applySameYearDescription}</div>
        <div class="full lh-lg">
	        <span class="title-c">計畫目標：</span><br>
	        ${plan.planGoal}
        </div>
        <div class="full lh-lg">
	        <span class="title-c">計畫內容：</span><br>
	        ${plan.planContent}
        </div>
        <div class="full">
      ...
      <div class="form">
        ...
        <div class="full lh-lg">
          <span class="title-c">質化效益：</span><br>
          ${plan.qualityBenefit}
        </div>
        <div class="full lh-lg">
      ...
      <div class="page">
        <button type="button" onclick="javascript:location.href='career_02'">返回修改</button>
        <button type="button" onclick="doPrint()">列印資料</button>
        
        
      <!--- Button trigger modal --->
      <button type="button" class="sendcase" data-bs-toggle="modal" data-bs-target="#sure">送出案件</button>
      <!---------- Modal ---------->
...
<script src="js/career_07/career_07.js"></script>
</body>
</html>