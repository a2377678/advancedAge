...
<script src="js/teach_payment_02/teach_payment_02.js"></script>
...
<div class=""> 
  ...
  <%@ include file="countDownComponent.jsp" %>
    ...
    <div class="form">
      ...
      <input type="number" id="approveAmounts" name="approveAmounts" value="${base.allowanceApproveAmounts }" step="1" min="0" size="50%" disabled required>
      </div>
      ...
      <div class="btn_box">
      <button class="save" onclick="save()">暫存資料</button>
      </div>  
    ...
    <div class="form">
      ...
      <textarea rows="5" id="planExecutionSituation" name="planExecutionSituation" onkeyup="wordsLimit(this,100)" placeholder="含課程內容、時數、受益人次等  ( 輸入框可拖曳加大範圍 )" required>${result.planExecutionSituation.replace("\\n","&#13;&#10;") }</textarea>
      </div>
        
      <div class="full">
      <label for="result">實施成果</label>
      <textarea rows="5" id="result" name="result" onkeyup="wordsLimit(this,100)" placeholder="含效益、特色、影響、服務對象滿意度，檢視是否達成預期指標及效益  ( 輸入框可拖曳加大範圍 )" required>${result.result.replace("\\n","&#13;&#10;") }</textarea>
      </div>
        
      <div class="full">
      <label for="suggestion">檢討與建議</label>
      <textarea rows="5" id="suggestion" name="suggestion" onkeyup="wordsLimit(this,100)" placeholder=" ( 輸入框可拖曳加大範圍 )" required>${result.suggestion.replace("\\n","&#13;&#10;") }</textarea>
      </div>
